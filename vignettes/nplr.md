...
Note that if neither $T_0$ nor *Ctrl* are provided, the default behaviour of *convertToProp()* is to adjust the values as proportions of their *min* and $m\textit{max*. In that case, the user should be aware that $y = 0.5$ does not correspond to the *IC50*, but to the *EC50*, as shown in the examples below.
