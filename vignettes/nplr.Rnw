...
\vspace{.5in}
As there are replicates, we can compare the effect of the different weighted methods with the default $residuals weights$, on the fitting. A 'no-weight' condition can be tested by setting the \texttt{LPweight} argument to 0: The vector of weights is then a vector of 1's.
